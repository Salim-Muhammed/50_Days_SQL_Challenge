...
select 
	...
	dense_rank() over(partition by product_category order by sum(total_sales) desc) as drnk
	from sales_data
	where extract(month from month)=1
	group by seller_id,product_category) as x
	where drnk <= 3;
...
select 
	...
	market_place,
	sum(total_sales) as sales,
	dense_rank() over(partition by market_place order by sum(total_sales) desc) as drnk
	from sales_data
	group by seller_id,market_place) as x
	where drnk <= 3;
